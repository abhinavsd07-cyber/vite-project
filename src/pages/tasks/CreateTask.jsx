...
export const CreateTask = () => {
    ...
    return (
        <div className="flex flex-col h-full bg-[#f8f9fa] w-full p-4 md:p-6 animate-fade-in relative z-10 overflow-auto">
            {/* Form Section Card */}
            <div className="bg-white rounded-[14px] border border-slate-200/80 shadow-[0_1px_5px_rgba(0,0,0,0.02)] mb-6 overflow-hidden">
                {/* Header Tabs */}
                <div className="px-6 pt-6 pb-2 border-b border-slate-100/60">
                        ...
                        {["SINGLE FILE UPLOAD", "MULTI FILE UPLOAD"].map((tab) => (
                            <button
                                ...
                                className={`pb-4 text-[12px] font-bold tracking-wider transition-all relative whitespace-nowrap ${
                                    activeUploadTab === tab ? "text-slate-900" : "text-slate-400 hover:text-slate-600"
                                }`}
                            >
                                {tab}
                                {activeUploadTab === tab && (
                ...
                <div className="p-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-6">
                        {/* Title */}
                        <div>
                            <label className="block text-[12px] font-medium text-slate-400 mb-2 uppercase tracking-wide">Title</label>
                            <input 
                                type="text" 
                                placeholder="Enter title"
                                className="w-full px-4 py-2.5 bg-white border border-slate-200 rounded-lg text-[13.5px] text-slate-700 focus:outline-none focus:ring-1 focus:ring-slate-300 placeholder:text-slate-300 transition-all font-light"
                            />
                        </div>

                        {/* Reference Number */}
                        <div>
                            <label className="block text-[12px] font-medium text-slate-400 mb-2 uppercase tracking-wide">Reference Number</label>
                            <input 
                                type="text" 
                        ...
                        {/* Category */}
                        <div>
                            <label className="block text-[12px] font-medium text-slate-400 mb-2 uppercase tracking-wide">Category</label>
                            <div className="relative">
                                <select className="w-full px-4 py-2.5 bg-white border border-slate-200 rounded-lg text-[13.5px] text-slate-700 focus:outline-none focus:ring-1 focus:ring-slate-300 transition-all font-light appearance-none">
                        ...
                        {/* Customer */}
                        <div>
                            <label className="block text-[12px] font-medium text-slate-400 mb-2 uppercase tracking-wide">Customer</label>
                            <div className="relative">
                                <select className="w-full px-4 py-2.5 bg-white border border-slate-200 rounded-lg text-[13.5px] text-slate-700 focus:outline-none focus:ring-1 focus:ring-slate-300 transition-all font-light appearance-none">
                        ...
                        <div className="md:row-span-2">
                            <label className="block text-[12px] font-medium text-slate-400 mb-2 uppercase tracking-wide">Remarks</label>
                            <textarea 
                                placeholder="Enter remarks"
                                rows={6}
                                className="w-full px-4 py-2.5 bg-white border border-slate-200 rounded-lg text-[13.5px] text-slate-700 focus:outline-none focus:ring-1 focus:ring-slate-300 placeholder:text-slate-300 transition-all font-light resize-none h-[115px]"
                            ></textarea>
                        </div>

                        {/* Document Upload */}
                        <div className="md:row-span-2">
                            <label className="block text-[12px] font-medium text-slate-400 mb-2 uppercase tracking-wide flex items-center">
                                Document Upload <span className="text-red-500 ml-1 mt-1">*</span>
                            </label>
                            <div className="border-2 border-dashed border-slate-200 rounded-xl p-8 flex flex-col items-center justify-center bg-slate-50/10 cursor-pointer hover:bg-slate-50 transition-colors h-[115px]">
                                <div className="bg-slate-50/50 p-2 rounded-lg mb-2">
                                    <Upload size={20} className="text-slate-400" />
                                </div>
                                <p className="text-[12.5px] text-slate-500 text-center font-medium">Drop files here from your device or click to upload</p>
                                <p className="text-[11px] text-slate-400 text-center mt-1">( Support format .CSV, .XLSX; max file size 5 MB )</p>
                            </div>
                        </div>

                        {/* Assign To */}
                        <div>
                            <label className="block text-[12px] font-medium text-slate-400 mb-2 uppercase tracking-wide">Assign To</label>
                            <div className="relative">
                                <select className="w-full px-4 py-2.5 bg-white border border-slate-200 rounded-lg text-[13.5px] text-slate-700 focus:outline-none focus:ring-1 focus:ring-slate-300 transition-all font-light appearance-none">
                    ...
                    <div className="flex items-center justify-end gap-3 mt-4">
                        <button className="px-6 py-2 border border-slate-200 rounded-lg text-[13px] font-bold text-slate-500 hover:bg-slate-50 transition-colors uppercase tracking-wide">Clear</button>
            ...
            <div className="bg-white rounded-[14px] border border-slate-200/80 shadow-[0_1px_5px_rgba(0,0,0,0.02)] flex-1 flex flex-col overflow-hidden">
                <div className="px-6 py-5 border-b border-slate-50 bg-white sticky top-0 z-10">
                    <h2 className="text-[14px] font-bold text-slate-900 uppercase tracking-widest">ALL TASKS</h2>
                </div>
                        ...
                        <tbody>
                            {MOCK_TASKS.map((task, index) => (
                                <tr key={task.id} className="border-b border-slate-100/60 transition-colors hover:bg-slate-50/50">
                                    ...
                                    <td className="py-4 px-2 text-[12.5px] text-slate-600 truncate max-w-[200px]" title={task.title}>{task.title}</td>
                                    <td className="py-4 px-2 text-[12.5px] text-slate-600 font-medium">{task.user}</td>
                                    <td className="py-4 px-2">
                                        <StatusBadge status={task.status} />
                                    </td>
                                    <td className="py-4 px-2 text-[12.5px] text-slate-600">{task.customer}</td>
                                    <td className="py-4 px-2 text-[12.5px] text-slate-600">{task.category}</td>
                                    <td className="py-4 px-2 text-[12.5px] text-slate-600">{task.date}</td>
