...
const MOCK_DATA = [
  { id: 1, name: "FY 2023-2024", start: "01/04/2023", end: "31/03/2024", status: "Active" },
  { id: 2, name: "FY 2022-2023", start: "01/04/2022", end: "31/03/2023", status: "Inactive" },
];
  ...
  return (
    <div className="flex flex-col h-full bg-[#f8f9fa] w-full animate-fade-in pb-12">
      <div className="px-6 py-5 shrink-0 flex items-center gap-4">
        ...
        <h1 className="text-xl font-bold text-slate-800">Financial Years</h1>
      </div>
            ...
            <table className="w-full text-left border-collapse min-w-[700px]">
              <thead>
                <tr className="border-b border-slate-100 bg-slate-50/50">
                  ...
                  <th className="py-4 px-6 text-[13px] font-semibold text-slate-600 whitespace-nowrap">Year Name</th>
                  <th className="py-4 px-6 text-[13px] font-semibold text-slate-600 whitespace-nowrap">Start Date</th>
                  <th className="py-4 px-6 text-[13px] font-semibold text-slate-600 whitespace-nowrap">End Date</th>
              ...
              <tbody>
                {data.map((item, index) => (
                  <tr key={item.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors group">
                    ...
                    <td className="py-3 px-6 text-[13px] text-slate-500">{item.start}</td>
                    <td className="py-3 px-6 text-[13px] text-slate-500">{item.end}</td>
                    <td className="py-3 px-6 text-[13px]">
                       <span className={`px-2.5 py-1 rounded-full font-medium text-xs ${item.status === 'Active' ? 'bg-emerald-50 text-emerald-600' : 'bg-slate-100 text-slate-600'}`}>
                          {item.status}
                       </span>
                    </td>
